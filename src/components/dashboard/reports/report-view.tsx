...
export function ReportView({ 
  ...
  const getCategoryType = useMemo(() => (catName: string) => {
      const mainCat = categories.find(c => c && c.name === catName);
      if (mainCat) return mainCat.type;
      const parentCat = categories.find(c => c && c.subcategories?.some(s => s.name === catName));
      return parentCat?.type;
  }, [categories]);
  ...
  const secondaryAccountSummary = useMemo(() => {
    ...
    const incomeCategories = categories.filter(c => c && c.type === 'income');
    const bankExpenseCategories = categories.filter(c => c && c.type === 'bank-expense');
    
    const postBankCat = categories.find(c => c && c.name.toLowerCase() === 'post bank');
    const postBankSubcategories = postBankCat?.subcategories?.map(s => s.name) || [];

    const combinedPostBankCategories = postBankSubcategories.length > 0 
        ? postBankSubcategories
        : Array.from(new Set([
            ...bankExpenseCategories.map(c => c.name), 
        ...
        {isPostBank && (
            <Card>
                ...
                <CardContent>
                    ...
                    <div className="mb-6">
                       <h4 className="text-sm font-bold uppercase mb-2 text-green-600 border-b pb-1">Income Categories Summary</h4>
                       <Table className="text-xs">
                           <TableHeader><TableRow><TableHead>Category</TableHead><TableHead className="text-right">Credit</TableHead><TableHead className="text-right">Debit</TableHead><TableHead className="text-right">Balance</TableHead></TableRow></TableHeader>
                           <TableBody>
                              {s.postBankAccordionData.filter(cat => getCategoryType(cat.name) === 'income').map(cat => (
                                 <TableRow key={cat.name}>
                                     <TableCell className="font-medium">{cat.name}</TableCell>
                                     <TableCell className="text-right font-mono text-green-600">{formatCurrency(cat.totalCredit)}</TableCell>
                                     <TableCell className="text-right font-mono text-red-600">{formatCurrency(cat.totalDebit)}</TableCell>
                                     <TableCell className="text-right font-mono">{formatCurrency(cat.balance)}</TableCell>
                                 </TableRow>
                              ))}
                           </TableBody>
                       </Table>
